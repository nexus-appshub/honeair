...
code = """package com.example.download
...
object MediaDownloader {
    ...
    private fun registerReceiverIfNeeded(context: Context) {
        if (!isReceiverRegistered) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                context.applicationContext.registerReceiver(downloadReceiver, filter, Context.RECEIVER_NOT_EXPORTED)
            } else {
                context.applicationContext.registerReceiver(downloadReceiver, filter)
            }
    ...
    fun downloadFileFromService(
        ...
        val job = coroutineScope.launch(Dispatchers.IO) {
            try {
                val targetFile = if (url.lowercase().contains("m3u8")) {
                    downloadHls(
                        context = context,
                        m3u8Url = url,
                        fileName = fileName,
    ...
    private fun downloadStandardFile(
        ...
        finalResponse.use { resp ->
            ...
            if (bytesCopied < 50000 && !contentType.contains("video") && !contentType.contains("octet-stream")) {
                targetFile.delete()
                throw Exception("Downloaded file is empty or invalid. If this was a direct download, the link might have expired. Please use one of the In-App Downloader Options (1-4) to select quality and download.")
            }
        }
...
with open("app/src/main/java/com/example/download/MediaDownloader.kt", "w", encoding="utf-8") as f:
    f.write(code)

print("Generated MediaDownloader.kt successfully!")
